  ...
  return (
    <div id="saved-favorites-view" className="px-4 py-3 space-y-4 pb-24 animate-in fade-in duration-200">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-extrabold text-slate-900">Saved PGs</h1>
      ...
      {favoritePGs.length === 0 ? (
        <div className="bg-white rounded-3xl p-8 text-center space-y-3 border border-slate-200/80 shadow-sm mt-4">
          ...
          <button
            ...
            className="px-5 py-2.5 bg-purple-600 text-white rounded-xl text-xs font-bold hover:bg-purple-700 shadow-md transition-all"
          >
            Explore PGs
          </button>
        </div>
        ...
        <div className="space-y-3">
          {favoritePGs.map((pg) => {
            ...
            return (
              <div
                key={pg.id}
                className="bg-white rounded-2xl p-3.5 border border-slate-200/80 shadow-sm flex gap-3.5 items-center group relative hover:border-purple-300 transition-all"
              >
                <div
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between">
                    <h3
                      onClick={() => onSelectPG(pg)}
                      className="font-bold text-sm text-slate-900 truncate cursor-pointer hover:text-purple-600"
                    >
                      {pg.name}
                    </h3>
                    <button
      ...
      {showCompareModal && comparedPGObjects.length >= 2 && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-lg rounded-3xl p-5 shadow-2xl space-y-4 max-h-[85vh] overflow-y-auto">
            <div className="flex justify-between items-center border-b pb-3">
              <h2 className="font-extrabold text-base text-slate-900">Side-by-Side PG Comparison</h2>
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              {comparedPGObjects.map((pg) => (
                <div key={pg.id} className="bg-slate-50 rounded-2xl p-3 border border-slate-200 space-y-2">
                  <img src={pg.images[0]} alt={pg.name} className="w-full h-24 object-cover rounded-xl" />
                  <div className="font-extrabold text-sm text-slate-900">{pg.name}</div>
                  <div className="text-purple-700 font-bold text-sm">₹{pg.pricePerMonth}/mo</div>
                  <div className="text-slate-500">Deposit: ₹{pg.costBreakdown.securityDeposit} (100% Refundable)</div>
                  <div className="text-slate-500">Food: {pg.foodIncluded ? '✓ 3 Meals Included' : 'Optional'}</div>
                  <div className="text-slate-500">Category: {pg.category.toUpperCase()}</div>
                  <div className="text-slate-500">Rating: ⭐ {pg.rating}</div>
                  <button
                    onClick={() => {
                      setShowCompareModal(false);
                      onSelectPG(pg);
                    }}
                    className="w-full py-2 bg-purple-600 text-white font-bold rounded-xl text-xs"
                  >
                    View & Book
                  </button>
                </div>
